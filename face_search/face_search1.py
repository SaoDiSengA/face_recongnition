...
if len(test_faces) != 0:
    shape = predictor(img_face, test_faces[0])  # img：一个numpy ndarray，包含8位灰度或RGB图像   box：开始内部形状预测的边界框   返回值：68个关键点的位置
...
for filename in os.listdir("sources"):
    ...
    if len(test_faces) != 0:
        shape = predictor(img, db_faces[0])  # img：一个numpy ndarray，包含8位灰度或RGB图像   box：开始内部形状预测的边界框   返回值：68个关键点的位置
        db_face_feature_list.append(face_reco_model.compute_face_descriptor(img, shape))

for i in range(len(db_face_feature_list)):
    e_distance = return_euclidean_distance(test_face_feature_list[0], db_face_feature_list[i])
    if e_distance < 0.5:
        print("匹配")
        img = cv2.imread("sources" + "/" + "img_" + str(i) + ".png")
        cv2.imshow("1", img)
        cv2.waitKey(2000)
    else:
        print("不匹配")
