...
class Face_Register:
    ...
    def pre_work_mkdir(self):
        ...
        if os.path.isdir(self.path_photos_from_camera):  # 用于判断某一对象(需提供绝对路径)是否为目录
            pass
        else:
            os.mkdir(self.path_photos_from_camera)  # 生成文件夹

    # 删除之前存的人脸数据文件夹 / Delete old face folders
    def pre_work_del_old_face_folders(self):
        # 删除之前存的人脸数据文件夹, 删除 "/data_faces_from_camera/person_x/"...
        folders_rd = os.listdir(self.path_photos_from_camera)  # 此方法返回一个列表，其中包含有指定路径下的目录和文件的名称
        for i in range(len(folders_rd)):
            shutil.rmtree(self.path_photos_from_camera + folders_rd[i])  # shutil.rmtree() #递归地删除文件
        if os.path.isfile("data/features_all.csv"): #用于判断某一对象(需提供绝对路径)是否为文件
    ...
    def check_existing_faces_cnt(self):
        if os.listdir("data/data_faces_from_camera/"):
            ...
            for person in person_list:
                person_num_list.append(int(person.split('_')[-1])) # -1表示数组的最后一个值
